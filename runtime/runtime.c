...
int is_bool(pyobj val) {
    return val.tag == BOOL_TAG;
}

int is_float(pyobj val) {
    return val.tag == FLOAT_TAG;
}
...
static int is_pyobj_equal(pyobj a, pyobj b) {
    if (a.tag == b.tag) {
        if (a.tag == BIG_TAG) {
            return a.ptr == b.ptr;
        } else {
...
static pyobj* list_subscript(list ls, pyobj n)
{
    ...
    case INT_TAG: {
        ...
        if (0 <= i && i < list_len)
            return &(ls.data[i]);
        else if (0 <= list_len + i && list_len + i < list_len)
            return &(ls.data[list_len + i]);
        else {
